  ...
  def add(mimeType: String):Box[Photo] = {
    ...
    if (findByName(name).isEmpty) {
      ...
      a.name(name + ".jpg")
        ...
        .source(Props.get("recipe.photo.source").openOr("src/main/webapp/photos/") + name + ".jpg")
        .minSource(Props.get("recipe.photo.source").openOr("src/main/webapp/photos/") + name + "_200x200.jpg")
        .mimeType(mimeType)
      saveBox(a)
    } else {
    	add(mimeType)
